...
start = 499; % horizon when starting at 0.3 s
...
%         pause
        ...
        if iter>1 && mean(Merit_end)>0
            [u_prop,fval,exitflag,output] = quadprog((H+H')/2+(0.8*eye(size((H+H')/2,1))),F',[Aineq],[bineq],[],[],lb,ub,[],options);      %
        elseif iter==1
            [u_prop,fval,exitflag,output] = quadprog((H+H')/2+0.8*eye(size((H+H')/2,1)),F',[Aineq],[bineq],[],[],lb,ub,[],options);      %
        ...
        suf = 0;
        scaling = 1;
        while suf==0 && iter>5 && isempty(u_prop)==0
            u_app = u_opt(:,n)+scaling*(u_prop-u_opt(:,n));
            ...
            for k = 1:ending
                ...
                x(2*n-1:2*n,k+1) = As(2*n-1:2*n,2*n-1:2*n)*x(2*n-1:2*n,k)+Bs(2*n-1:2*n,1:2)*u_app(2*k-1:2*k);
                y(n,k) = x(2*n,k) + R0(n)*(u_app(2*k-1,1)+u_app(2*k,1)) + Vemf(n);
            end
            ...
            u_test(:,n) = u_app;
            for i=1:ending
                w_hat(i) = sum(u_test(2*i-1,1:N))/N;
                error_lambda(n,i) = (u_test(2*i-1,n)-w_hat(i));
                error_mu(i) = (y(1:N,i)'*u_test(2*i,1:N)'+Rb*ones(1,N)*(u_test(2*i,1:N).^2)');
                error_nu(i) = ((y(1:N,i)'*u_test(2*i-1,1:N)')-P(i));       
            end
            
            merit(iter,n) = sum(error_mu(error_mu>0))+sum(error_nu(error_nu>0));
            
            if merit(iter,n)<=merit(iter-1,n)+scaling
                suf = 1;
            elseif scaling < 0.1
                suf = 1;
            else
            ...
            u_opt(:,n) = scaling*u_prop;
        end
        
        if isempty(u_prop)==0 && iter<6
            u_opt(:,n) = u_prop;
        end
        ...
        merit(iter,n) = sum(error_mu(error_mu>0).^2)+sum(error_nu(error_nu>0).^2);
...
%             nu(i) = max([1e-6,nu(i)+gamma_nu*(sum(mu_error_saved(n:N:(N-1)*iter+n,1:min(10,i))>0)).*error_nu(i)]);              
    ...
    %% x and y
        ...
        if sum(mu_error_saved(N*iter,:)<=0)==numel(mu_error_saved(iter,:)) && sum(nu_error_saved(N*iter,:)<=0)==numel(nu_error_saved(iter,:))
